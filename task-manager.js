...
function createTask(event) {
    ...
    const taskStorageList = JSON.parse(localStorage.getItem('taskStorage')) || [];
    ...
    const task = { taskId, taskName, taskDescription, taskIcon, taskCheckListArray, taskDeadlineInput };
    taskList.push(task);
    taskStorageList.push(task);

    window.localStorage.setItem('task', JSON.stringify(taskList));
    window.localStorage.setItem('taskStorage', JSON.stringify(taskStorageList));
...
function renderTaskList() {
    ...
    for (const task of taskList) {
        ...
        const { taskId, taskName, taskDescription, taskIcon, memberName, taskCheckListArray, taskDeadlineInput } = task;
        ...
        for(var i = 0; i < taskCheckListArray.length; i++) {
            ...
            if (taskCheckListArray[i].checked == false) {
                taskChecklistDiv += `<input name="${taskCheckListArray[i].checkPointName}" onclick="checklistStatus(${taskId}, ${i}, true, 'task'); move(${taskId}, 'task', 'ongoingTask'); renderTaskList(); " type="checkbox">
                <label contentEditable="true" oninput="changeCheckpoint(${taskId}, ${i}, event, 'task');" for="${taskCheckListArray[i].checkPointName}"> ${taskCheckListArray[i].checkPointName}</label>`
            }
...
=======
            ...
            unstartedTasks.appendChild(taskElement);
>>>>>>> e6ff636381f1aa44d69ff12caa81e2b8e20e653d
    }
}
...
function renderArchiveList(){
    ...
    for(const ar of archiveList){
        ...
        const { taskId, taskName, taskDescription, taskIcon, memberName, memberId, taskCheckListArray, taskDeadlineInput } = ar;
        var taskChecklistDiv = "";
        for(var i = 0; i < taskCheckListArray.length; i++) {
            if (taskCheckListArray[i].checked == true) {
